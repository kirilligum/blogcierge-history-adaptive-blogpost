...
async function handleRagMode(
  ...
  if (!db || !vectorIndex || !ai) {
    const errorDetail = "CRITICAL: RAG database, vector index, or AI binding is not configured.";
    ...
    return new Response(JSON.stringify({ error: "Server configuration error: RAG system not available." }), { status: 500 });
  }
